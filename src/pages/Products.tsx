...
export default function Products() {
  ...
  const filters = [
    ...
    { key: 'seeds', label: t.products.filter.seeds },
    { key: 'spices', label: t.products.filter.spices },
    { key: 'dehydrated', label: t.products.filter.dehydrated },
  ];
  ...
  return (
    <main className="pt-20">
      ...
      {activeFilter !== 'all' && categoryInfo[activeFilter as keyof typeof categoryInfo] && (
        <section className="py-10 bg-white border-b border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-4">
              <span className="text-4xl">{categoryInfo[activeFilter as keyof typeof categoryInfo].emoji}</span>
              <div>
                <h2 className={`text-2xl font-bold text-gray-900 ${isRTL ? 'font-heading-ar' : 'font-heading-en'}`}>
                  {categoryInfo[activeFilter as keyof typeof categoryInfo].title}
                </h2>
                <p className="text-gray-500 text-sm mt-1">
                  {categoryInfo[activeFilter as keyof typeof categoryInfo].desc}
                </p>
              </div>
            </div>
          </div>
