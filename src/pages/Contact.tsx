...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const inquiries = JSON.parse(localStorage.getItem('alraheeq-inquiries') || '[]');
    inquiries.push({ ...form, timestamp: new Date().toISOString() });
    localStorage.setItem('alraheeq-inquiries', JSON.stringify(inquiries));
  };
              ...
              <div className="glass-card rounded-3xl p-8 md:p-10 border border-brand-gold/15">
                {sent ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    >
                      {isRTL ? 'إرسال استفسار جديد' : 'Send Another Inquiry'}
                    </button>
                  </div>
                ) : (
                  <>
